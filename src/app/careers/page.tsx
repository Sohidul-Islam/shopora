...
import { Briefcase, MapPin, DollarSign, Calendar, FileText, CheckCircle2, User, Mail, Send } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#05060b] via-[#090b11] to-[#040508] py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-3">
          ...
          <h1 className="text-4xl font-black font-display text-white tracking-tight leading-none">Careers at Shopora</h1>
          <p className="text-sm text-slate-400">Help us design, scale, and deliver the standard for production-ready full-stack e-commerce software. Build things that matter with a world-class team.</p>
        </div>

        {/* Jobs List */}
        <div className="space-y-4">
          {jobsList.map((job) => (
            <div key={job.id} className="glass border border-slate-800/40 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 shadow-lg shadow-black/10 hover:border-slate-800 transition duration-300">
              <div className="space-y-3">
                <span className="text-[10px] bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-full py-1 px-2.5 font-black uppercase tracking-wider">{job.department}</span>
                <h3 className="text-xl font-bold font-display text-white">{job.title}</h3>
                
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-400 font-semibold">
        ...
        {selectedJob && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="glass border border-slate-800/50 rounded-3xl p-6 sm:p-10 max-w-xl w-full relative shadow-2xl shadow-black/80 space-y-6 max-h-[90vh] overflow-y-auto">
              <button 
              ...
              <div className="space-y-1.5 border-b border-slate-900 pb-3">
                <span className="text-[10px] text-blue-400 font-black uppercase tracking-wider">{selectedJob.department}</span>
                <h3 className="text-2xl font-black font-display text-white leading-none">{selectedJob.title}</h3>
              ...
              {applied ? (
                <div className="py-12 text-center space-y-4">
                  ...
                  <h3 className="text-lg font-bold text-white font-display">Application Received!</h3>
                  <p className="text-xs text-slate-400">Our HR recruitment team will review your profile and contact you shortly.</p>
                </div>
              ) : (
                <form onSubmit={handleApplySubmit} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-black text-slate-400 tracking-wider">Resume Link</label>
                    <input 
                      type="url" 
                      required
                      value={form.resume}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-black text-slate-400 tracking-wider">Cover Letter (Optional)</label>
                    <textarea 
                      rows={4}
