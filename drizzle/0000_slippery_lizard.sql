...
ALTER TABLE `variant_attribute_values` ADD CONSTRAINT `var_attr_val_var_id_fk` FOREIGN KEY (`variant_id`) REFERENCES `product_variants`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `variant_attribute_values` ADD CONSTRAINT `var_attr_val_attr_val_id_fk` FOREIGN KEY (`attribute_value_id`) REFERENCES `attribute_values`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
